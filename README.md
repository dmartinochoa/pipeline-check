...
| **Harness CI/CD** | Harness pipeline YAML (`.harness/`) | `--harness-path` | 5 checks · `HARNESS-001..005` · step image digest pinning, untrusted `<+codebase.*>` / `<+trigger.*>` expression injection into step commands, privileged steps, literal secrets in pipeline / stage variables, pipe-to-shell installs |
...
└── core/
    ...
    └── checks/
        ...
        ├── harness/rules/     # HARNESS-001 .. HARNESS-005 — Harness CI/CD pipeline YAML (image pinning, untrusted-expression command injection, privileged steps, literal secrets in variables, pipe-to-shell)
