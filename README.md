...
| **CircleCI** | `.circleci/config.yml` | `--circleci-path` | 37 checks · `CC-001..037` · incl. Go-module-verification bypass, the model-load triad (`trust_remote_code`, unpinned model revision, unsafe pickle deserialization), untrusted PR/build context into an agentic AI CLI |
