...
**1110+ checks** across **32 providers**, mapped to **18 compliance standards**, with **111 autofixers**, plus **52 attack chains** correlating findings into MITRE ATT&CK-mapped kill chains. A dataflow taint engine catches multi-step and cross-job propagation that single-rule scanners miss.
...
| **GitLab CI** | `.gitlab-ci.yml` | `--gitlab-path` | 39 checks (`GL-001`--`037`, plus `TAINT-004` and `TAINT-008`). `GL-037` flags a pipeline `variables:` / `script:` that disables Go module verification. |
...
|------|---------|-------------|
| `--pipeline` / `-p` | `auto` | `auto` (detect from cwd), `aws`, `azure_cloud`, `gcp`, `terraform`, `cloudformation`, `pulumi`, `github`, `gitea`, `gitlab`, `bitbucket`, `azure`, `jenkins`, `circleci`, `cloudbuild`, `buildkite`, `drone`, `tekton`, `argo`, `argocd`, `dockerfile`, `kubernetes`, `helm`, `oci`, `scm`, `npm`, `pypi`, `maven`, `nuget`, `composer`, `cargo`, `gomod`, `rubygems` |
...
└── core/
    ...
    └── checks/
        ...
        ├── github/rules/      # GHA-001 .. GHA-073, GHA-086..115 + TAINT-001..003, TAINT-009
        ├── gitlab/rules/      # GL-001 .. GL-037 + TAINT-004 / TAINT-008
