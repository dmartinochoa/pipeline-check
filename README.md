...
| **GitHub Actions run forensics** | Live Actions REST API | `--pipeline runs` | 4 checks · `RUN-001..004` · audits run history for what actually executed (fork-originated runs, privileged-trigger runs that fired, and via `--audit-runs-logs` secrets leaked in run logs plus fork runs that minted a cloud OIDC token) vs. what the static config could do |
...
└── core/
    ...
    └── checks/
        ...
        ├── runs/rules/        # RUN-001 .. RUN-004 — GitHub Actions run-history forensics via the live Actions REST API (fork-originated runs, privileged-trigger runs that fired, secrets leaked in run logs, fork runs that minted a cloud OIDC token)
