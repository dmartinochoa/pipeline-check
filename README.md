...
| **npm** | `package.json` / `package-lock.json` / `npm-shrinkwrap.json` | `--npm-path` | 6 checks (`NPM-001`--`006`): floating version ranges, lockfile entries missing `integrity`, non-registry sources (git+ssh, http://, git+https without 40-char SHA pin), install-time lifecycle scripts (`preinstall` / `install` / `postinstall` / `prepare`), git deps using mutable refs, and known-compromised package versions (curated `_compromised_packages.py` registry seeded with event-stream / ua-parser-js / coa / rc / node-ipc). Skips `node_modules/`. |
...
└── core/
    ...
    └── checks/
        ...
        ├── npm/rules/         # NPM-001 .. NPM-006 — package.json + package-lock.json supply-chain hygiene + curated compromised-package registry
