...
**1200+ checks** across **38 providers**, mapped to **18 compliance standards**, with **111 autofixers**, plus **56 attack chains** correlating findings into MITRE ATT&CK-mapped kill chains. A dataflow taint engine catches multi-step and cross-job propagation that single-rule scanners miss.
...
```
                 +-----------+
  Config files   |  Scanner  |   1200+ checks across 38 providers
...
└── core/
    ...
    └── checks/
        ...
        ├── scm_org/rules/     # ORG-001 .. ORG-004 — GitHub organization-wide governance via the REST API (org-wide 2FA requirement, default member repository permission, Actions allow-list, default workflow token permission); complements scm (one repo) with org-level posture
