...
| **Harness CI/CD** | Harness pipeline YAML (`.harness/`) | `--harness-path` | 4 checks · `HARNESS-001..004` · step image digest pinning, untrusted `<+codebase.*>` / `<+trigger.*>` expression injection into step commands, privileged steps, literal secrets in pipeline / stage variables |
...
└── core/
    ...
    └── checks/
        ...
        ├── harness/rules/     # HARNESS-001 .. HARNESS-004 — Harness CI/CD pipeline YAML (image pinning, untrusted-expression command injection, privileged steps, literal secrets in variables)
