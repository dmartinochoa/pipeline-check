...
| **GitLab pipeline run forensics** | Live GitLab REST API | `--pipeline gitlab_runs` | 4 checks · `GLRUN-001..004` · audits pipeline history for what actually executed (merge-request pipelines that fired, and via `--audit-runs-logs` fork merge-request pipelines that ran untrusted code, secrets leaked in fork job traces, fork pipelines that minted a cloud OIDC token) vs. what the static `.gitlab-ci.yml` could do |
...
| **Harness CI/CD** | Harness pipeline YAML (`.harness/`) | `--harness-path` | 9 checks · `HARNESS-001..009` · step image digest pinning, untrusted `<+codebase.*>` / `<+trigger.*>` expression injection into step commands, privileged steps, literal secrets in pipeline / stage variables, pipe-to-shell installs, TLS-verification bypass, sensitive host-path mounts, untrusted context into an agentic AI CLI, AI output autolanding without review |
...
└── core/
    ...
    └── checks/
        ...
        ├── gitlab_runs/rules/ # GLRUN-001 .. GLRUN-004 — GitLab pipeline run-history forensics via the live GitLab REST API (merge-request pipelines that fired, fork-MR pipelines that ran untrusted code, secrets leaked in fork job traces, fork pipelines that minted a cloud OIDC token)
        ...
        ├── harness/rules/     # HARNESS-001 .. HARNESS-009 — Harness CI/CD pipeline YAML (image pinning, untrusted-expression command injection, privileged steps, literal secrets in variables, pipe-to-shell, TLS bypass, host-path mounts, AI prompt injection, AI autoland)
