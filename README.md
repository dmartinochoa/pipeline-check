...
| **Developer environment** | `.vscode/` · `.devcontainer/` · `.claude/settings.json` · `.mcp.json` | `--devenv-path` | 7 checks · `DEV-001..007` · config that auto-executes on repo open (folder-open tasks, devcontainer lifecycle, committed Claude Code hooks, remote fetch-and-execute, executable-path / env hijack, committed MCP command servers) |
