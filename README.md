...
| **npm** | `package.json` / `package-lock.json` / `npm-shrinkwrap.json` / `.npmrc` | `--npm-path` | 7 checks (`NPM-001`--`007`): floating version ranges, lockfile entries missing `integrity`, non-registry sources (git+ssh, http://, git+https without 40-char SHA pin), install-time lifecycle scripts (`preinstall` / `install` / `postinstall` / `prepare`), git deps using mutable refs, known-compromised package versions (curated `_compromised_packages.py` registry seeded with event-stream / ua-parser-js / coa / rc / node-ipc), and `.npmrc` missing or disabling `ignore-scripts=true` (the file-side complement to DF-024). Skips `node_modules/`. |
...
└── core/
    ...
    └── checks/
        ...
        ├── npm/rules/         # NPM-001 .. NPM-007 — package.json + package-lock.json + .npmrc supply-chain hygiene + curated compromised-package registry
