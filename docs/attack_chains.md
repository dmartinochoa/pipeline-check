...
A workflow uses unpinned third-party actions (GHA-001), checks out PR head on a ``pull_request_target`` trigger (GHA-002), and carries literal secrets in the YAML (GHA-008). Any one of those is exploitable; the combination gives a fork-PR attacker two independent code-execution paths to the same plaintext credentials.
