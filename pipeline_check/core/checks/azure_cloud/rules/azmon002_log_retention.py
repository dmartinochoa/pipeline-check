...
def check(catalog: ResourceCatalog) -> list[Finding]:
    ...
    for setting in settings:
        ...
        any_enabled = False
        # ``days == 0`` means "retain indefinitely" (the LARGEST
        # retention), so it never lowers the minimum; only finite values
        # constrain it. Tracking the smallest FINITE value keeps a single
        # indefinite category from masking a short-retention sibling.
        ...
        for log_entry in logs:
            ...
            if rp and getattr(rp, "enabled", False):
                ...
                if days != 0 and (min_finite is None or days < min_finite):
                    min_finite = days
        if not any_enabled:
            passed = True
            desc = (
        ...
        elif min_finite is None:
            # Every enabled category retains indefinitely (days=0).
            passed = True
            desc = (
                f"Diagnostic setting '{name}' retains logs indefinitely "
                "(days=0 means no expiry)."
            )
        elif min_finite >= _MIN_RETENTION_DAYS:
            passed = True
            desc = (
                f"Diagnostic setting '{name}' retains logs for "
                f"{min_finite} days (>= {_MIN_RETENTION_DAYS})."
            )
        else:
            passed = False
            desc = (
                f"Diagnostic setting '{name}' retains logs for only "
                f"{min_finite} days (minimum: {_MIN_RETENTION_DAYS})."
            )
