...
RULE = Rule(
    ...
    exploit_example=(
        ...
        "    - step:\n"
        "        script:\n"
        "          - pip install git+https://bitbucket.org/acme/helper.git\n"
        "\n"
        ...
        "# pipeline with its repository variables. `file:` paths and\n"
        "# bare tarball URLs share the hole.\n"
        "\n"
        "# Safe: pin the dependency to an immutable commit SHA.\n"
        "          - pip install git+https://bitbucket.org/acme/helper.git@<full-sha>"
    ),
)
