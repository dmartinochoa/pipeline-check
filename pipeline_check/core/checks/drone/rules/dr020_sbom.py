"""DR-020. Pipeline should produce an SBOM (syft / cyclonedx)."""
...
RULE = Rule(
    ...
    docs_note=(
        "Detection mirrors GHA-007 / BK-010 / CC-007 / TKN-010, the shared "
        "SBOM-token catalog (syft, cyclonedx, spdx, bom, trivy sbom) is "
        "searched across every string in the pipeline document. The rule "
        "only fires on artifact-producing pipelines (``docker build`` / "
        "``docker push`` / ``buildah`` / ``kaniko`` / etc.) so lint / "
        "test-only pipelines don't trip it. The Drone analog of BK-010 / "
        "TKN-010."
    ),
)
