"""DR-022. Pipeline should run a vulnerability scanner."""
...
RULE = Rule(
    ...
    recommendation=(
        ...
        "dependency tree, and fail the build on findings above your "
        "threshold so known CVEs don't ship to production silently."
    ),
    docs_note=(
        "Detection mirrors GHA-020 / BK-012 / CC-020 / TKN-012, the shared "
        "scanner-token catalog (trivy, grype, snyk, clair, npm audit, "
        "pip-audit, etc.) is searched across every string in the pipeline "
        "document. Fires on any pipeline that runs no scanner (the build "
        "ships without a CVE signal). The Drone analog of BK-012 / TKN-012."
    ),
)
