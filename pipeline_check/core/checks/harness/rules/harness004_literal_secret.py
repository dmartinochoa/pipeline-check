...
from ..base import HarnessPipeline, iter_variables
...
RULE = Rule(
    ...
    docs_note=(
        "Fires on a pipeline-level or stage-level ``variables:`` entry whose "
        "``value`` is a credential-shaped literal (matched by the shared "
        "secret-shape catalog, ``find_secret_values``) rather than a "
        "``<+secrets.getValue(...)>`` expression. ``type: Secret`` variables "
        "and any ``<+...>`` expression value are skipped (those are managed "
        "references, not literals); empty values are ignored. The value is "
        "redacted in the finding. Same value-shape model as the literal-"
        "secret rules across the other providers (DR-004 / BK-002 / "
        "TKN-005)."
    ),
    exploit_example=(
...
def check(pipeline: HarnessPipeline) -> Finding:
    ...
    desc = (
        "No pipeline / stage variable holds a literal credential."
        if passed else
