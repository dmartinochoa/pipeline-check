"""HARNESS-015. Pipeline should sign produced artifacts (cosign / sigstore)."""
...
RULE = Rule(
    ...
    docs_note=(
        "Detection mirrors GHA-006 / BK-009 / CC-006 / TKN-009 / DR-019, "
        "the shared signing-token catalog (cosign, sigstore, "
        "slsa-github-generator, slsa-framework, notation-sign) is searched "
        "across every string in the pipeline document. The rule only fires "
        "on artifact-producing pipelines (``docker build`` / ``docker "
        "push`` / ``buildah`` / ``kaniko`` / etc.) so lint / test-only "
        "pipelines don't trip it. The Harness analog of BK-009 / TKN-009."
    ),
)
