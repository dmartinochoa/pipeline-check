"""HARNESS-017. Pipeline should produce SLSA provenance attestation."""
...
RULE = Rule(
    ...
    recommendation=(
        ...
        "Harness's built-in SLSA provenance / attestation step, so a "
        "verifier can confirm which pipeline and source revision produced "
        "the artifact."
    ),
    docs_note=(
        "Detection mirrors GHA-024 / BK-011 / CC-024 / TKN-011 / DR-021, "
        "the shared provenance-token catalog (slsa, provenance, in-toto, "
        "attestation, cosign attest) is searched across every string in the "
        "pipeline document. The rule only fires on artifact-producing "
        "pipelines (``docker build`` / ``docker push`` / ``buildah`` / "
        "etc.) so lint / test-only pipelines don't trip it. The Harness "
        "analog of BK-011 / TKN-011."
    ),
)
