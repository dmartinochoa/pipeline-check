...
PROVIDERS_AND_FLOORS: dict[str, tuple[str, int]] = {
    "github":     ("pipeline_check.core.checks.github.rules",     50),
    "gitlab":     ("pipeline_check.core.checks.gitlab.rules",     55),
    "bitbucket":  ("pipeline_check.core.checks.bitbucket.rules",  55),
    "azure":      ("pipeline_check.core.checks.azure.rules",      20),
    "jenkins":    ("pipeline_check.core.checks.jenkins.rules",    35),
    "circleci":   ("pipeline_check.core.checks.circleci.rules",   45),
    "cloudbuild": ("pipeline_check.core.checks.cloudbuild.rules", 80),
    "dockerfile": ("pipeline_check.core.checks.dockerfile.rules", 10),
    "kubernetes": ("pipeline_check.core.checks.kubernetes.rules", 10),
}
...
def test_at_least_half_of_ci_providers_cross_60_percent():
    ...
    for provider in ci_providers:
        ...
        if 100 * len(matched) / max(1, len(rule_ids)) >= 60:
            above_60 += 1
    # As of v0.4.0 the count is 0; bump this floor as backfill lands.
    assert above_60 >= 0, (
